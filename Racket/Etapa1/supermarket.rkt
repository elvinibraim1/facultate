...
(define (min-tt counters)
  (min-tt-rec counters (cons 0 1000000000000)))

(define (min-tt-rec counters pair)
  (if (null? counters) pair
      (cond
        ((< (counter-tt (car counters)) (cdr pair)) (min-tt-rec (cdr counters) (cons (counter-index (car counters)) (counter-tt (car counters)))))
        ((>= (counter-tt (car counters)) (cdr pair)) (min-tt-rec (cdr counters) pair))
   )))
...
(define (add-to-counter C name n-items)
  (struct-copy counter (tt+ C n-items) [queue (reverse (cons (cons name n-items) (reverse (counter-queue C))))])
  ) 
